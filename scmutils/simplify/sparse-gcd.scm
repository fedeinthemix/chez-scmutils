...
(define *heuristic-sparse-gcd-win* (make-parameter 0))
(define *heuristic-sparse-gcd-lose* (make-parameter 0))
...
(define (sparse-heuristic-gcd p1 p2 n ds win lose)
  (let lp ((trials 0) (args (generate-list n interpolate-random)))
	...
	(begin (*heuristic-sparse-gcd-win*
...
;;; FBE: make a parameter
(define *interpolate-primes-stream* (make-parameter '()))
...
(define (make-interpolation-args k)
  (let lp ((i 0) (s (*interpolate-primes-stream*)) (args '()))
    (if (fix:= i k)
	(begin (*interpolate-primes-stream* s)
	       args)
...
|#

;;; FBE: calls 'sparse-gcd' with wrong argument count! -> comment out
...
(define (reset-heuristic-sparse-gcd-test)
  ...
  (*heuristic-sparse-gcd-enabled* #t))
