...
;;; FBE: factorial is also defined in kernel/numeric.scm.  Therefore
;;; we rename the version defined here to int:factorial and use it in
;;; this file.
;; (define (factorial n)
;;   (if (int:< n 2)
;;       1
...
;; 	  -1)
;;       0))
(define (int:factorial n)
  (if (int:< n 2)
      1
      (int:* n (int:factorial (int:- n 1)))))

(define number-of-permutations int:factorial)

(define (number-of-combinations  n k)
  (int:quotient (int:factorial n)
		(int:* (int:factorial (int:- n k))
		       (int:factorial k))))
