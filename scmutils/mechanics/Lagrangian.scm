...
(define* (path->state-path q #:optional n)
  ...
  (lambda (t)
    (list->vector
     (cons t
	   (cons (g:apply q (list t)) ; FBE: (q t) -> (g:apply q (list t))
...
(define* (Lagrange-equations Lagrangian #:optional dissipation-function)
  (lambda (q)
    (let ((state-path (Gamma q)))
      (if (default-object? dissipation-function)
          (- (D (compose ((partial 2) Lagrangian) state-path))
...
;;; FBE start 
(define* (Lagrangian->acceleration L #:optional dissipation-function)
  (lambda (state)
    (if (default-object? dissipation-function)
        (let ((P ((partial 2) L))
              (F ((partial 1) L)))
