...
;;; FBE: make these parameters
(define *max-tableau-depth* (make-parameter (void)))
(define *max-tableau-width* (make-parameter (void)))
(define bulirsch-stoer-steps (make-parameter (void)))
(define bulirsch-stoer-magic-vectors (make-parameter (void)))
  ...
  (define (bsi n)
    (cons-stream (expt 2 (+ n 1))
		 (cons-stream (* 3 (expt 2 n))
			      (bsi (+ n 1)))))
  (*max-tableau-depth* max-depth)
  (*max-tableau-width* max-width)
...
(define %numerics-ode-bulirsch-stoer-dummy-1
...
(define (bulirsch-stoer-floating-lisptran f n error-measure)
  (let ((mm (vector-Gragg f n))
	...
	(let ((modified-midpoint (mm state delta-t)))
	  ...
	  (let m-loop ((m 1)
		       ;;(old-verr) ; FBE must give a value
		       (old-verr unspecific)
...

(define %numerics-ode-bulirsch-stoer-dummy-2
 (add-integrator!
