...
;;; FBE start: move expressions before "define" to after them.
	...
	(else
	 (lambda arguments
	   (if (not (acceptable-arglist? arguments arity))
	       (error ":wrong-number-of-arguments" operator arity arguments)) ; FBE
	   (apply (general-find-handler arguments)
		  arguments)))))

    (set! default-operation
      (if (default-object? default-operation)
	  ;; (named-lambda (no-handler . arguments) ; FBE named-lambda -> lambda
	  (lambda arguments
