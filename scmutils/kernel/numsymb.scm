...
;;; FBE: make it a parameter. Also replace its use in this file and in
;;; simplify/simplify.scm
;;; calculus/dgutils.scm
...
(define %kernel-numsymb-dummy-9
 (addto-symbolic-operator-table '= symb:=))

(define (symb:zero? x)
  (cond ((number? x) (zero? x))
	(else `(= ,:zero ,x))))
(define %kernel-numsymb-dummy-10
 (addto-symbolic-operator-table 'zero? symb:zero?))

(define (symb:one? x)
  (cond ((number? x) (one? x))
	(else `(= ,:one ,x))))
(define %kernel-numsymb-dummy-11
...
(define %kernel-numsymb-dummy-12
...
(define %kernel-numsymb-dummy-13
...
(define %kernel-numsymb-dummy-14
...
(define %kernel-numsymb-dummy-15
...
(define (symb:abs x)
  (cond ((number? x) (abs x))
	(else `(abs ,x))))
(define %kernel-numsymb-dummy-16
	...
	((number? e)
	 (cond ((zero? e) :one)
		...
		(symb:/ :one (symb:expt b (- e))))
	       (else `(expt ,b ,e))))
	(else `(expt ,b ,e))))
(define %kernel-numsymb-dummy-36
...
(define %kernel-numsymb-dummy-17
...
(define %kernel-numsymb-dummy-18
...
(define %kernel-numsymb-dummy-19
...
(define %kernel-numsymb-dummy-20
		...
		(else
		 (let ((s (n:sqrt exp)))
		   (if (exact? s)
		       ...
		       `(sqrt ,exp))))))
      `(sqrt ,exp)))
(define %kernel-numsymb-dummy-21
...
(define (symb:exp x)
  (if (number? x)
	  ...
	  (if (zero? x)
	      ...
	      `(exp ,x)))
      `(exp ,x)))
(define %kernel-numsymb-dummy-22
...
(define (symb:log x)
  (if (number? x)
	  ...
	  (if (one? x)
	      ...
	      `(log ,x)))
      `(log ,x)))
(define %kernel-numsymb-dummy-23
...
(define %kernel-numsymb-dummy-24
...
(define %kernel-numsymb-dummy-25
...
(define %kernel-numsymb-dummy-26
...
(define (symb:csc x)
  (if (number? x)
	  ...
	  (if (zero? x)
	      ...
	      `(/ 1 ,(symb:sin x))))
      `(/ 1 ,(symb:sin x))))
(define %kernel-numsymb-dummy-27
...
(define (symb:sec x)
  (if (number? x)
	  ...
	  (if (zero? x)
	      ...
	      `(/ 1 ,(symb:cos x))))
      `(/ 1 ,(symb:cos x))))
(define %kernel-numsymb-dummy-28
  ...
  (if (null? opts)
	...
	(if (number? y)
		...
		(if (number? x)
		    ...
		    `(atan ,y ,x)))
	    `(atan ,y ,x)))))
(define %kernel-numsymb-dummy-29
...
(define (symb:asin x)
  (if (number? x)
	  ...
	  (if (zero? x)
	      ...
	      `(asin ,x)))
      `(asin ,x)))
(define %kernel-numsymb-dummy-30
...
(define (symb:acos x)
  (if (number? x)
	  ...
	  (if (one? x)
	      ...
	      `(acos ,x)))
      `(acos ,x)))
(define %kernel-numsymb-dummy-31
...
(define (symb:cosh x)
  (if (number? x)
	  ...
	  (if (zero? x)
	      ...
	      `(cosh ,x)))
      `(cosh ,x)))
(define %kernel-numsymb-dummy-32
...
(define (symb:sinh x)
  (if (number? x)
	  ...
	  (if (zero? x)
	      ...
	      `(sinh ,x)))
      `(sinh ,x)))
(define %kernel-numsymb-dummy-33
...
(define (symb:max . l)
  (if (for-all? l number?)
      (apply max l)
      `(max ,@l)))
(define %kernel-numsymb-dummy-34
...
(define (symb:min . l)
  (if (for-all? l number?)
      (apply min l)
      `(min ,@l)))
(define %kernel-numsymb-dummy-35
